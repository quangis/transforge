...
class TestAlgebra(unittest.TestCase):
    ...
    def test_serial(self):
        ...
        self.assertQuery(alg, graph, STEPS(C, b2c, B, a2b, A),
            results={TEST.wf1})
        self.assertQuery(alg, graph, STEPS(C, B, A),
            results={TEST.wf1})
        self.assertQuery(alg, graph, STEPS(b2c, a2b),
            results={TEST.wf1})
        self.assertQuery(alg, graph, STEPS(C, a2b, B, b2c, A),
            results=None)
        self.assertQuery(alg, graph, STEPS(B, b2c, C, a2b, A),
            results=None)
    ...
    def test_serial_skip(self):
        ...
        self.assertQuery(alg, graph, [c2d, a2b],
            results={TEST.wf1})
        self.assertQuery(alg, graph, STEPS(c2d, b2c, a2b),
            results={TEST.wf1})
        self.assertQuery(alg, graph, [D, b2c, A],
            results={TEST.wf1})
        self.assertQuery(alg, graph, STEPS(D, b2c, A),
            results=None)
    ...
    def test_parallel(self):
        ...
        self.assertQuery(alg, graph, [D, bc2d, AND(B, C, a2b, b2c)],
            results={TEST.wf2})
        self.assertQuery(alg, graph, [D, bc2d, AND(STEPS(B, a2b), STEPS(C, b2c))],
            results={TEST.wf2})
        self.assertQuery(alg, graph, [D, bc2d, AND(A, B)],
            results={TEST.wf2})
        self.assertQuery(alg, graph, STEPS(D, bc2d, AND(A, B)),
            results=None)
    ...
    def test_sequenced_skips(self):
        ...
        self.assertQuery(lang, graph, STEPS(C, a2b), results=set())
