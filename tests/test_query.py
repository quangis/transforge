...
from transformation_algebra.query import Query, OR, AND, STEP
...
class TestAlgebra(unittest.TestCase):
    ...
    def test_serial(self):
        ...
        self.assertQuery(alg, graph, STEP(C, b2c, B, a2b, A),
            results={TEST.wf1})
        self.assertQuery(alg, graph, STEP(C, B, A),
            results={TEST.wf1})
        self.assertQuery(alg, graph, STEP(b2c, a2b),
            results={TEST.wf1})
        self.assertQuery(alg, graph, STEP(C, a2b, B, b2c, A),
            results=None)
        self.assertQuery(alg, graph, STEP(B, b2c, C, a2b, A),
            results=None)
    ...
    def test_serial_skip(self):
        ...
        self.assertQuery(alg, graph, [c2d, a2b],
            results={TEST.wf1})
        self.assertQuery(alg, graph, STEP(c2d, b2c, a2b),
            results={TEST.wf1})
        self.assertQuery(alg, graph, [D, b2c, A],
            results={TEST.wf1})
        self.assertQuery(alg, graph, STEP(D, b2c, A),
            results=None)
    ...
    def test_parallel(self):
        ...
        self.assertQuery(alg, graph, [D, bc2d, AND(B, C, a2b, b2c)],
            results={TEST.wf2})
        self.assertQuery(alg, graph, [D, bc2d, AND(STEP(B, a2b), STEP(C, b2c))],
            results={TEST.wf2})
        self.assertQuery(alg, graph, [D, bc2d, AND(A, B)],
            results={TEST.wf2})
        self.assertQuery(alg, graph, STEP(D, bc2d, AND(A, B)),
            results=None)
    ...
    def test_choice(self):
        ...
        self.assertQuery(lang, graph, STEP(D, bc2d, OR(A, D)),
            results=None)
        ...
        self.assertQuery(lang, graph, STEP(D, bc2d, OR(STEP(B, a2b), STEP(C, a2b))),
            results={TEST.wf1})

        # Choice between operations in non-last place
        self.assertQuery(lang, graph, STEP(D, bc2d, OR(b2c, b2c2), B),
            results={TEST.wf1, TEST.wf2})
    ...
    def test_sequenced_skips(self):
        ...
        self.assertQuery(lang, graph, STEP(C, a2b), results=set())
